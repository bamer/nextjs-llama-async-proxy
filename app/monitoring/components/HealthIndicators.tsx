...
import { Timer, NetworkCheck, CheckCircle, Info, Warning } from "@mui/icons-material";
import { formatUptime } from "../utils/monitoring-utils";
...
export function HealthIndicators({ metrics, isDark }: HealthIndicatorsProps) {
  return (
    <Card sx={{
      background: isDark
      ...
      <CardContent>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              {metrics?.memory?.used && metrics.memory.used > 85
                ? <Warning color="error" />
                : <CheckCircle color="success" />}
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              {metrics?.cpu?.usage && metrics.cpu.usage > 90
                ? <Warning color="error" />
                : <CheckCircle color="success" />}
              <Typography variant="body2">
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              {metrics?.disk?.used && metrics.disk.used > 95
                ? <Warning color="error" />
                : <CheckCircle color="success" />}
              <Typography variant="body2">
                Disk: {metrics?.disk?.used ?? 0}%
              </Typography>
            </Box>
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
