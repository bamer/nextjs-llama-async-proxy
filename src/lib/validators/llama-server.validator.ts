...
export const llamaServerConfigSchema = z
  .object({
    ...
    threads: z.number().int().min(-1).default(-1).describe("Number of threads (-1 for auto)"),
    gpu_layers: z.number().int().min(-1).default(-1).describe("Number of GPU layers (-1 for all)"),
  })
  .strict()
...
export const llamaServerSettingsSchema = z
  .object({
    ...
    threads: z.number().int().min(-1).describe("Number of threads"),
    gpu_layers: z.number().int().min(-1).describe("GPU layers"),
  })
  .strict()
