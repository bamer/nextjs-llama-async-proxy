...
- **Pattern**: When catching errors or receiving error responses:
  ...
  console.error("[MODULE] Operation failed:", response.error);
  showNotification(`Save failed: ${response.error}`, "error");
...
3. **DeCentralized Cadence** - Single server timer, not per-client timers
...
```javascript
// In socket handlers
console.log("[DEBUG SERVER] Event received:", { event: "models:scan", data: req });
console.log("[DEBUG SERVER] Scan result:", { found: files.length, path: modelsDir });
console.error("[DEBUG SERVER] Error details:", error.message);

// In scan/discover operations
console.log("[DEBUG SERVER] Directory exists:", dirExists);
console.log("[DEBUG SERVER] Files in directory:", files.length, files);
```

### Frontend (public/js/)

Use `[DEBUG CLIENT]` prefix consistently:

```javascript
// Controllers
console.log("[DEBUG CLIENT] Controller created");
console.log("[DEBUG CLIENT] Controller init");
console.log("[DEBUG CLIENT] Controller willUnmount");

// State operations
console.log("[DEBUG CLIENT] State changed:", key, value);
console.log("[DEBUG CLIENT] API request:", event, data);
console.log("[DEBUG CLIENT] API response:", data);

// Component lifecycle
console.log("[DEBUG CLIENT] Component created, props:", props);
console.log("[DEBUG CLIENT] Component willReceiveProps:", newProps);
console.log("[DEBUG CLIENT] Button clicked:", action);
