...
###0️⃣### Mandatory :  Workspace Initialization
...
Check if ###0️⃣### have been done already and ###1️⃣### have been understand.

1️⃣  Receive Mission
   └─► If ambiguous → ask human → restart step 1

2️⃣  Analyze Mission → Build a Detailed TODO (atomic tasks)
...
COMPLETION :
```json
{ "taskId": string, "status": "COMPLETION", "artifacts": [string], "timestamp": string }	
```
    -Agent finished its work; artifacts are ready for review.

ERROR :	
```json
{ "taskId": string, "status": "ERROR", "error": string, "stack": string, "timestamp": string }	
```
    -Unrecoverable failure – orchestrator must log and re‑assign.

REASSIGN :	
```json
{ "taskId": string, "reason": string, "timestamp": string }	
```
    -Agent needs the task back (e.g., reviewer rejected).

APPROVE (Reviewer only) :
```json
{ "taskId": string, "action": "APPROVE", "comments": string, "timestamp": string }	
```
REJECT (Reviewer only)	
```json
{ "taskId": string, "action": "REJECT", "reason": string, "timestamp": string }	
```
PASS (Tester only)	
```json
{ "taskId": string, "action": "PASS", "coverage": { … }, "timestamp": string }	
```
FAIL (Tester only)
```json
{ "taskId": string, "action": "FAIL", "missingMetrics": [string], "coverage": { … }, "timestamp": string }	
```
DOCS_READY (Docs only)	
```json
{ "taskId": string, "updatedFiles": [string], "summary": string, "timestamp": string }	
```
CLEANUP_DONE (Janitor only)	
```json
...
```
###7### The log is immutable – never edited after written. It is the single source of truth for post‑mortems, CI debugging, and audit trails.
```json
{
