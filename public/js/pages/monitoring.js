...
class MonitoringController {
  ...
  init() {
    this.unsubs.push(
      ...
      stateManager.subscribe(
        "metricsHistory",
        (h) => this.comp && this.comp.setState({ history: h })
      )
    );
    this.load();
  }
  ...
  render() {
    this.comp = new MonitoringPage({
      metrics: stateManager.get("metrics"),
  ...
  render() {
    ...
    return Component.h(
      ...
      Component.h(
        ...
        Component.h(
          "select",
          { className: "time-select", "data-action": "time" },
...
class MetricCards extends Component {
  render() {
    ...
    const cards = [
      {
        title: "CPU",
        val: `${(m.cpu?.usage || 0).toFixed(1)}%`,
        icon: "💻",
        color: m.cpu?.usage > 80 ? "danger" : "success",
      },
      {
        title: "Memory",
        val: window.AppUtils?.formatBytes?.(m.memory?.used || 0) || "0 B",
        icon: "🧠",
        color: "info",
      },
      {
        title: "Disk",
        val: `${(m.disk?.used || 0).toFixed(1)}%`,
        icon: "💾",
        color: m.disk?.used > 80 ? "danger" : "info",
      },
    ...
    if (m.gpu) {
      cards.push({
        title: "GPU Memory",
        val: window.AppUtils?.formatBytes?.(m.gpu.memoryUsed || 0) || "0 B",
        icon: "🎮",
        color: "info",
      });
      cards.push({
        title: "GPU Usage",
        val: `${(m.gpu.usage || 0).toFixed(1)}%`,
        icon: "🎯",
        color: m.gpu.usage > 80 ? "danger" : "info",
      });
    }
    ...
    return Component.h(
      ...
      Component.h(
        ...
        data.length > 0
          ? Component.h(
            "svg",
            { viewBox: "0 0 800 200", className: "line-chart" },
...
class SystemHealth extends Component {
  render() {
    ...
    return Component.h(
      ...
      Component.h(
        "div",
        { className: `status ${healthy ? "good" : "warning"}` },
  ...
  _check(m) {
    return (
      !m || ((m.cpu?.usage || 0) <= 80 && (m.memory?.used || 0) <= 85 && (m.disk?.used || 0) <= 90)
    );
  }
  _checkItem(name, val, thresh) {
    ...
    return Component.h(
      "div",
      { className: `check ${ok ? "good" : "warning"}` },
