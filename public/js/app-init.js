...
function showErrorBoundary(error) {
  ...
  function initializeSocket() {
    try {
      ...
      // IMPORTANT: Register handler BEFORE calling connect() to avoid race condition
      socketClient.on("connect", () => {
        console.log("[App] Socket.IO connected, subscribing to metrics...");
        socketClient.request("metrics:subscribe", { interval: 2000 });
      });

      // Also handle case where socket is already connected
      if (socketClient.isConnected) {
        console.log("[App] Socket already connected, subscribing to metrics...");
        socketClient.request("metrics:subscribe", { interval: 2000 });
      }
