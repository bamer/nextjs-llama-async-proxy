import { apiClient } from '@/utils/api-client';
import axios from 'axios';

jest.mock('axios');

describe('ApiClient', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe('get', () => {
    it('returns success response on successful request', async () => {
      const mockData = { data: 'test' };
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        get: jest.fn().mockResolvedValue({ data: mockData }),
      } as any);

      const result = await apiClient.get('/test');
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
      expect(result.timestamp).toBeDefined();
    });

    it('returns error response on network error', async () => {
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        get: jest.fn().mockRejectedValue({ message: 'Network error', request: {} }),
      } as any);

      const result = await apiClient.get('/test');
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('NETWORK_ERROR');
    });

    it('returns error response on 404', async () => {
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        get: jest.fn().mockRejectedValue({
          message: 'Not Found',
          response: { status: 404, statusText: 'Not Found', data: {} },
        }),
      } as any);

      const result = await apiClient.get('/test');
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('404');
    });
  });

  describe('post', () => {
    it('returns success response on successful request', async () => {
      const mockData = { id: '123' };
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        post: jest.fn().mockResolvedValue({ data: mockData }),
      } as any);

      const result = await apiClient.post('/test', mockData);
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
      expect(result.timestamp).toBeDefined();
    });

    it('handles network error', async () => {
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        post: jest.fn().mockRejectedValue({ message: 'Network error', request: {} }),
      } as any);

      const result = await apiClient.post('/test', {});
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('NETWORK_ERROR');
    });
  });

  describe('put', () => {
    it('returns success response on successful request', async () => {
      const mockData = { id: '123', updated: true };
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        put: jest.fn().mockResolvedValue({ data: mockData }),
      } as any);

      const result = await apiClient.put('/test/123', mockData);
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
    });
  });

  describe('delete', () => {
    it('returns success response on successful request', async () => {
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        delete: jest.fn().mockResolvedValue({ data: { deleted: true } }),
      } as any);

      const result = await apiClient.delete('/test/123');
      expect(result.success).toBe(true);
      expect(result.data).toEqual({ deleted: true });
    });
  });

  describe('patch', () => {
    it('returns success response on successful request', async () => {
      const mockData = { id: '123', patched: true };
      const mockedAxios = axios as jest.Mocked<typeof axios>;
      mockedAxios.create.mockReturnValue({
        patch: jest.fn().mockResolvedValue({ data: mockData }),
      } as any);

      const result = await apiClient.patch('/test/123', mockData);
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
    });
  });
});
