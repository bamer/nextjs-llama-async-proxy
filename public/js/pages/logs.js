...
class LogsController {
  constructor(options = {}) { this.router = options.router || window.router; this.unsubs = []; this.comp = null; }
...
class LogsPage extends Component {
  render() {
    ...
    return Component.h("div", { className: "logs-page" },
      ...
      Component.h("div", { className: "logs-container" },
        filtered.length === 0 ? Component.h("p", { className: "empty" }, "No logs") :
          filtered.map(l => Component.h("div", { className: `log-entry level-${l.level || "info"}`, key: l.id || Math.random() },
  ...
  _getFiltered() {
    ...
    if (this.state.filters?.level && this.state.filters.level !== "all") ls = ls.filter(l => l.level === this.state.filters.level);
  ...
  getEventMap() {
    return {
      ...
      "click [data-action=clear]": () => { if (confirm("Clear all logs?")) stateManager.clearLogs().then(() => { stateManager.set("logs", []); showNotification("Logs cleared", "success"); }); },
