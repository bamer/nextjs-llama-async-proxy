			...
			Component.h("div", { className: "card settings-card" }, [
				...
				Component.h("div", { className: "form-grid inference-grid" }, [
					...
					this._renderField("batchSize", "Batch Size", "number", {
						...
						min: 1,
						max: 8192,
						helpText: "Logical batch size (CPU)",
					}),
					this._renderField("ubatchSize", "Physical Batch Size", "number", {
						value: c.ubatchSize,
						min: 1,
						max: 8192,
						helpText: "Physical batch size for GPU (--ubatch-size)",
					}),
				...
				// Memory Fit Options
				Component.h("h4", { className: "card-subtitle" }, "Memory Fit Options"),
				Component.h("div", { className: "form-row" }, [
					Component.h("label", { className: "checkbox-label" }, [
						Component.h("input", {
							type: "checkbox",
							id: "router-fitEnabled",
							"data-field": "fitEnabled",
							checked: c.fitEnabled !== false,
						}),
						Component.h("span", {}, "Enable auto-fit to device memory (--fit)"),
					]),
				]),
				Component.h("div", { className: "form-grid" }, [
					this._renderField("fitTarget", "Fit Target (MiB)", "number", {
						value: c.fitTarget,
						min: 256,
						max: 16384,
						helpText: "Memory margin per device for fit",
					}),
					this._renderField("fitCtx", "Fit Min Context", "number", {
						value: c.fitCtx,
						min: 512,
						max: 32768,
						step: 512,
						helpText: "Minimum context size when fitting",
					}),
				]),
